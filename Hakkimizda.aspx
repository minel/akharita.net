<%@ Page Title="" Language="C#" MasterPageFile="~/Main.master" AutoEventWireup="true" CodeFile="Hakkimizda.aspx.cs" Inherits="Hakkimizda" %>

<asp:Content ID="Content1" ContentPlaceHolderID="CphMidContents" Runat="Server">
    <div class="pages-head">
	<h1>Hakkımızda</h1>
	<h3>Değerlerimiz ve disiplinimiz, kalite ve üretkenliğimizin temel anahtarıdır.</h3>
</div>
<div class="hakkimizda-contains">

	<div class="hakkimiz">
		<h2>Vizyonumuz</h2>
		<span>
			2006 yılında kurulan AkHarita
			profesyonel ekibiyle mühendislik
			firmalarına haritacılıkla ilgili
			hizmetlerine ihtiyaç duyan diğer
			tüm müşterilere hizmet sunmak
			amacıyla kurulmuştur.
			<br />
			Gerek inşaat sektörü gerek
			Alüminyum sektöründe birçok firmaya
			çözüm ortaklığında bulunmuştur.
			<br />
			Bu amaçla şirketimiz, donanımını ve kadrosunu
			daima zengin tutmayı hedeflemiştir.
			Kuruluşundan bu yana daima teknolojik yenilikleri
			ve mesleki alandaki gelişmeleri takip etmiş
			ve müşterilerine iyi ve kaliteli hizmet
			verme amacını taşımıştır. 
		</span>
	</div>

	<div class="hakkimiz">
		<h2>Misyonumuz</h2>
		<span>
			Müşteri memnuniyetini temel alan hizmetler ve çözümler sunmak.
			<br />
			
			Şirketimiz, çağın gereklerine uygun olarak kaliteli hizmet,
			doğru ve güvenilir veri üretme konusunda bu güne kadar göstermiş
			olduğumuz titizliğimizi devam ettirerek daha ileriyi hedefleyeceğiz.
		</span>
	</div>
</div>
</asp:Content>

